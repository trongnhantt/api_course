...
public interface CourseRepository extends JpaRepository<Course, Long> {
    @Query("SELECT c FROM Course c WHERE c.student.studentId = :studentId")
    List<Course> findByStudentId(@Param("studentId") String studentId);
    
    @Query("SELECT c FROM Course c WHERE c.student.studentId = :studentId AND c.status = :status")
    List<Course> findByStudentIdAndStatus(@Param("studentId") String studentId, @Param("status") String status);
    
    @Query("SELECT c FROM Course c WHERE c.student.studentId = :studentId AND c.semester = :semester")
